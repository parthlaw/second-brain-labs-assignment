...
const ProjectScreen = () => {
  ...
  const { data, isLoading } = useQuery({
    queryFn: () => {
      return getOneProject({
        token: cookies['access-token'].token as string,
        data: { projectId: parseInt(id as string) },
      })
    },
    queryKey: ['project'],
  })
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: (data: any) => {
      if (!data || !data.success) {
        setMessages([
          ...messages,
          { sender: 'other', text: 'Error Occured in getting the response' },
        ])
        return
      }
  ...
  const handleSendMessage = () => {
    ...
    mutate({
      ...
      data: { projectId: parseInt(id), message: newMessage },
    })
  }
